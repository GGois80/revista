...
<?php exit();
      endif ; if($_POST['vsenha'] != $_POST['senha']){ ?>
                  <script>
                      alert('Repita a senha a senha Novamente!');
                          location.href='cad-jorn.php';
                  </script>
<?php             exit();
              };
...
if ($imagem["name"]) {
    ...
    <?php exit();
        ...
        $query = "INSERT INTO usuario(nome,sexo,email,foto,cel,senha,flag,moderuser) VALUES ('$nome','$sexo','$email','$nome_imagem','$cel','$senha',2,1)";
        $upa = mysqli_query($conexao,$query);

        // Se os dados forem inseridos com sucesso, retorna essa mensagem
        if ($upa){
            $_SESSION['cadastrado-sucesso'] = true; ?>
            <script>
                alert('Cadastrado com Sucesso!!');
                location.href='cad-jorn.php';//e redireciono o usuario para a pagina inicial, se quiserem mandar pra listagem fiquem a vontade
            </script>
        <?php exit();
          } else{?>
                <script>
                    alert('Erro ao cadastrar usuario');
                    location.href='proc-cadjor.php';
                </script>
            <?php exit();
        }
    }
